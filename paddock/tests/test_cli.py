...
def test_a_data_dir_outside_a_repo_resolves_to_itself(tmp_path: Path) -> None:
    """Tolerant, not fatal: `--data-dir` may name a directory with no repo above it, and `pin()` degrades from there with a caveat rather than the CLI refusing to start."""
