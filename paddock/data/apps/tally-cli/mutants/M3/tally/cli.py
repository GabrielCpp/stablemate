...
def commit_or_preview(path: Path, data: dict, dry_run: bool) -> bool:
    ...
    and another for `import`. A dry run leaves the real ledger byte-for-byte as it was, and
    writes the ledger it would have produced to a `.preview` sibling so the caller can diff
    exactly what a wet run would land.
    """
    if dry_run:
