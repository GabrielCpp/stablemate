...
def build_parser() -> argparse.ArgumentParser:
    ...
    Each command declares every flag it takes, `--file` included, so a command's whole usage
    reads off its own `--help` in one place and adding a command never means auditing a shared
    namespace for options it does not honour.
    """
