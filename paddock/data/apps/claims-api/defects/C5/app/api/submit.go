...
func (s *Server) SubmitClaim(w http.ResponseWriter, r *http.Request) {
	...
	// Answered directly rather than through the conversion, so the created claim is the one
	// that was just stored rather than a copy of it.
	writeJSON(w, http.StatusCreated, map[string]any{"claim": map[string]any{
		"id":            claim.ID,
