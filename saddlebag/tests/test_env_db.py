...
@pytest.fixture
def web(pool: Pool):
    return pool.env_add(name="web-local", env="local", project="globex",
...
def test_a_duplicate_name_in_the_same_project_is_rejected(pool: Pool, web):
    with pytest.raises(PoolError, match="already exists"):
        pool.env_add(name="web-local", env="local", project="globex")
...
def test_env_find_scopes_to_one_project(pool: Pool, web):
    ...
    assert [e.name for e in pool.env_find("globex")] == ["web-local"]
...
def test_the_store_key_is_project_qualified(pool: Pool, web):
    assert web.store_key("VITE_FIREBASE_API_KEY") == "globex/env-001/VITE_FIREBASE_API_KEY"
