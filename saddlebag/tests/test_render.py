...
def web(pool: Pool, tmp_path: Path):
    ...
    environment = pool.env_add(name="web-local", env="local", project="globex",
                               target=str(tmp_path / ".env.local"))
    for entry in (
        EnvironmentEntry(key="PROJECT_ID", kind=KIND_CONFIG, value="globex"),
...
def seeded(pool: Pool, store, web):
    ...
    cred = pool.add(username="qa@globex.example", env="local", project="globex",
...
def test_username_and_password_refs_share_one_lease(pool: Pool, store, seeded):
    ...
    assert resolution.values["TEST_USER_EMAIL"] == "qa@globex.example"
...
def test_a_failed_lease_hands_back_the_ones_taken_beside_it(pool: Pool, store, seeded):
    ...
    other = pool.add(username="b@x.com", env="local", project="globex")
