...
def test_a_config_only_environment_never_opens_the_store(pool: Pool, tmp_path):
    """§7, and the whole reason config is first class: this must work on a container with no keyring, no Vault, and no configuration whatsoever."""
...
def test_dotenv_output_round_trips_through_the_parser(tmp_path):
    """The writer is the exact inverse of the reader — a secret that reads back differently than it was written is a corrupted secret."""
    values = {
        "PLAIN": "abc",
