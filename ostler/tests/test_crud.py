...
def test_create_epic_allocates_id_and_parses(tmp_path: Path):
    ...
    assert len(epic.eid.split("-", 1)[1]) == 26
...
def test_set_status_rewrites_only_the_field_not_the_prose(tmp_path: Path):
    """A story may legitimately write about status and about QA passing."""
    g = load(tmp_path)
...
def test_ensure_fixtures_adds_the_section_to_a_story_written_before_it_existed(tmp_path: Path):
    """The tolerant writer, for a migration."""
    doc = markdown.split(
        "---\ntype: story\nslug: editor\n---\n"
        "# Story: Editor\n\n## Dependencies\n\n(none)\n\n## Context\n\nWhy it matters.\n"
    )
...
def test_story_frontmatter_carries_the_allocated_id(repo: Path):
    """A story.md is read on its own constantly — by the coder workflow picking up work, by `ostler trace`, by a human opening the file."""
    crud.add_seed(load(repo), "epic-a", "s1", status="researched")
...
def test_story_id_uses_the_repo_prefix(tmp_path: Path):
    """Prefix is the first four letters of the repo name, uppercased — so `todo-app` mints TODO-n."""
...
def test_seed_layers_and_services_round_trip(repo: Path):
    ...
    assert seed.layers == ("frontend", "backend")
