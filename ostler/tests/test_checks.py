...
def test_text_is_identity_not_display() -> None:
    """Two spellings of one call render the same string, or the binding check refuses on whitespace and argument order instead of on substance."""
...
def test_a_soft_wrapped_bullet_parses_as_markdown_renders_it() -> None:
    """Books wrap bullets at a column; the break is a space to every reader but Python's grammar, so a wrapped `subject="…"` must bind as the one-line call it renders as."""
...
def test_canonical_text_parses_back_to_the_same_call() -> None:
    """`text()` is fed to authors and to refusal messages; a rendering the parser then rejects would send someone to fix a bullet by copying a spelling that cannot be written."""
    for value in (
...
def test_out_of_band_is_declared_independently_of_shape() -> None:
    """`out_of_band` and `observes` vary independently: `emitted` is single-observation and out-of-band, `persists` is paired-observation and out-of-band — a compiler reading both fields off the spec never needs to know either check by name."""
...
def test_every_spec_names_the_defect_it_excludes() -> None:
    """`excludes:` is the sentence a refusal quotes and the test of whether a check earns a place in the vocabulary at all — a check that excludes nothing is prose with parentheses."""
...
def test_a_lifecycle_claim_is_expressible_as_a_paired_observation() -> None:
    """`created` / `removed` exist so "it was created" is writable without collapsing into "it is there now" — the after-only spelling passes identically on a no-op."""
...
def test_every_declarable_check_is_observable_by_the_harness() -> None:
    """The two tables are spelled twice on purpose — `ostler.checks` says what may be declared, the stdlib-only harness says what observing it means — so nothing but a test stops them drifting."""
...
def test_every_type_that_states_claims_can_say_what_observing_one_looks_like() -> None:
    """A type with claims and no check key is covered unconditionally: `checksDeclared` is empty by construction, so whatever the scenario happened to assert fulfils it."""
...
def test_every_graded_key_is_a_declared_bullet() -> None:
    """The drift this closes: `status:`/`errors:`/`auth:` were graded on an endpoint for as long as the mapper existed and declared nowhere, so `fmt` could not place them and a `verify:` written under one bound to the `does:` above."""
...
def test_a_load_bearing_key_is_one_some_type_instruments() -> None:
    """`verify:` and `code:` are the keys an author expects machinery behind; `meaning:` is nobody's, and `parent:`/`detail:` are resolved on every type alike with no locator use, so neither is in the set."""
...
def test_the_expected_form_is_the_failing_checks_own_signature(value: str, expected: str) -> None:
    """The counter-case is a canned example: an author shown `http_status(code=…)` after mis-calling `absent` learns nothing about `absent`, and guesses again on the next lap."""
...
def test_a_signature_is_an_example_before_it_is_a_type() -> None:
    """The separator is the whole finding."""
    for spec in checks.CHECKS:
...
def test_a_call_whose_arguments_do_not_parse_is_still_a_named_check() -> None:
    """The name is recovered from the text, not from the parse — the parse is the thing that just failed, so a name read only out of a successful parse is unavailable in exactly the case that needs it, and every such author was handed all sixteen checks to re-choose."""
...
def test_only_a_refusal_that_named_no_check_falls_back_to_the_vocabulary() -> None:
    """No name recovered means no check chosen yet, so the answer is the menu — and *only* then."""
...
def test_the_one_of_rule_holds_on_both_sides_of_the_binding() -> None:
    """`parse_check` reads the book's bullet and `bind` reads the plan's recovered call."""
...
def test_an_unparsable_pattern_is_refused_at_bind_not_at_the_harness() -> None:
    """`matches=` is a regular expression, not free prose, and the two checks that declare it are the only ones the grammar marks that way."""
    for name, args in (
...
def test_only_a_declared_pattern_argument_is_compiled() -> None:
    """The flag, not the name, is what gates the compile: `omits`' `text=` is plain prose and must bind even though `[` alone is not a parsable regular expression."""
...
def test_omits_is_the_vocabularys_one_negative_observation() -> None:
    """Every other check asserts what a subject holds, and a clause about what a response may *not* contain has no positive form — so a book stating one had nothing to declare."""
...
def test_exit_status_binds_the_code_a_command_ends_with() -> None:
    """A tool's output is not its verdict: `created` and `json_path` can read what a process printed and never notice it printed it on the way to a non-zero exit."""
...
def test_json_path_equals_is_a_json_scalar(text: str, value: object) -> None:
    """The declared value keeps its JSON type through parse, bind and the canonical rendering, so a book's `equals=8250` reaches the harness as a number and reads back as the same bullet."""
...
def test_a_filter_segment_survives_the_declaration_gate_and_the_canonical_rendering() -> None:
    """The quote and the `==` inside a filter live inside the quoted `path=` string, which the `ast`-based parser reads as one literal — and the canonical text reads back as itself."""
...
def test_a_path_argument_is_the_same_call_with_or_without_the_root_token() -> None:
    """`$` is not a key — every resolver in the harness drops it before walking."""
...
def test_owning_keys_include_code_on_every_type_and_only_the_flagged_others() -> None:
    """`qa context` maps a diff through `owning_keys`; `code:` owns everywhere (a flow cites the code it is grounded in whether or not its profile lists the key), and the only other owners are the bullets the registry flags — a schema or a fixture file under its own name."""
    for uitype in registry.UI_TYPES:
...
def test_a_check_written_as_a_markdown_code_span_is_a_check() -> None:
    """Books render a call as code, and the backticks are the rendering."""
...
def test_a_value_holding_two_code_spans_is_not_unwrapped_to_its_middle() -> None:
    """The closing run has to match the opening one and nothing may sit outside it."""
...
def test_a_backticked_test_reference_reaches_the_test_reference_arm() -> None:
    """The silent half of the same defect, and the larger one: the suffix test read ``ts` `` rather than `ts`, so 16 misfiled paths were reported as unrecognisable values instead of as values on the wrong key."""
...
def test_the_message_never_names_a_destination_key() -> None:
    """`parse_check` sees only the value, never the node it sits in, so it cannot know which key a relocated value may legally land on — that is a property of the container, not of the value."""
...
def test_a_relocation_the_nodes_type_cannot_hold_falls_back_to_its_own_key() -> None:
    """`screen` declares `verify:` and never `tests:` (`registry.declared_keys`), so a caller that hands the node's own keys to `bullet` must not be offered `- tests: …`, a key `screen` has no way to carry."""
...
def test_a_relocation_the_nodes_type_does_hold_is_offered() -> None:
    """`concept` declares both `verify:` and `tests:`, so the same classification, handed `concept`'s own keys, still proposes the move — unaffected by the fallback the case above exercises."""
    refused = checks.parse_check("`e2e/dash_test.go::TestDash`")
    assert isinstance(refused, checks.Refusal)
