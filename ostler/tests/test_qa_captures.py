...
def test_a_reference_spelled_on_the_declaring_side_is_refused_by_name():
    """Stripping the `$` quietly would admit two spellings for one concept on the side that mints it."""
...
def test_every_name_this_accepts_is_one_a_reference_resolves_to():
    """The property, stated as a test rather than as a docstring: no accepted declaration may mint a name `references` would not hand back from the `$name` a later bullet writes."""
