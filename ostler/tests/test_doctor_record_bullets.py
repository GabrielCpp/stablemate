...
def test_a_bullet_key_buried_under_an_undeclared_container_is_reported(repo: Path) -> None:
    """`request:` is not a bullet key `endpoint` declares, so its `method:`/`path:` children fall back to the flat-subtree grammar and land in `meta["request"]` as strings — not as the top-level `method:`/`path:` bullets the type actually grades against."""
...
def test_a_backtick_quoted_child_key_is_still_read_as_the_node_bullet_it_spells(repo: Path) -> None:
    """The book quotes a child key in backticks as often as it writes it bare — `` `status`: 200 `` says the same key as `status: 200`."""
...
def test_a_buried_child_the_node_also_states_at_top_level_is_not_reported(repo: Path) -> None:
    """An undeclared container has no grammar of its own, so nothing here can tell whether a buried child restates the node's own claim or replaces it."""
    write(repo / ENDPOINT_PATH, _endpoint_book_with(
...
def test_a_declared_container_is_not_also_read_by_this_branch(repo: Path) -> None:
    """`response:` is declared and `record=True`, so `_check_record_properties` owns it already; this branch only ever looks at keys `endpoint` never wrote down."""
...
def test_a_prose_parent_spelled_like_no_bullet_key_never_fires(repo: Path) -> None:
    """The key-ish clause is load-bearing: without it, a numbered-list item whose 'key' is a sentence of prose would be misread as a container bullet, and the promote-to-top-level remedy would be wrong advice for it — that shape belongs to a different finding entirely."""
    write(repo / ENDPOINT_PATH, _endpoint_book_with(
