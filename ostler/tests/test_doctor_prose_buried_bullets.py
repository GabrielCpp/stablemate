...
def test_a_declared_multiword_key_is_never_reported(repo: Path) -> None:
    """`consistency rule` is `SHARED_NORMATIVE_KEYS`' own multi-word spelling — declared on every type — so using it as a container never reads as burial no matter what it nests."""
    write(repo / ENDPOINT_PATH, _endpoint_book_with(
...
def test_this_code_and_misnested_bullet_never_both_fire_on_the_same_child(repo: Path) -> None:
    """The two are gated by opposite readings of `_BULLET_KEY_SPELLING`, so one container can never trip both: a key-ish undeclared container (`request:`) is `misnested-bullet`'s, and a prose-shaped one (this test's) is this code's — never the same finding twice."""
    write(repo / ENDPOINT_PATH, _endpoint_book_with(
        "when the account is missing the lookup still runs",
        "consistency: a missing account never raises before the lookup completes",
        "verify: omits(subject=\"lookup error\", text=\"KeyError\")"))
