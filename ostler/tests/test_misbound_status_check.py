...
`uneven-claim-coverage` catches the *imbalance* a misbinding leaves behind (one claim
overcovered, a sibling with nothing); this catches the specific, provable case where the check
itself names a number absent from the claim it landed on but present, verbatim, on this node's
own `status:`/`exits:` bullet — the shape a book gets into by writing every claim first and
every `verify:` last, the pathology `paddock/data/apps/globex`'s `#get-health` shipped with.
"""
...
def test_one_finding_per_authored_bullet_when_the_first_verify_fans_out(repo: Path):
    """The `does: all` shape `post-widgets` shipped with: one `verify:` sits under a nested
    normative list with two children — so `attributed_checks` fans it out to both — and a
    second, correctly-bound `verify:` follows below, under its own `errors:` claim. Positional
    reconstruction sees this list as longer than the authored `verify:` bullets and reports the
    fanned check against the *second* bullet's index; the fix must report it against its own,
    the first, and must not also mistake the second, correctly-bound bullet for misbound.
    """
    write(repo / "docs/features/acme/http/api-service.md",
          "---\ntype: server\nslug: api-service\ntitle: API service\n---\n# API service\n\n"
          "## Endpoints\n\n### post-widgets\n"
...
def test_indexed_engine_agrees_with_attributed_checks(repo: Path):
    """`attributed_check_bullets` is a second projection of the same walk `attributed_checks`
    runs, not a second binder — the two must never disagree about which values are bound to
    the node's own contract or to which claim, only about whether the authored index rides
    along.
    """
    write(repo / "docs/features/acme/http/api-service.md",
          "---\ntype: server\nslug: api-service\ntitle: API service\n---\n# API service\n\n"
