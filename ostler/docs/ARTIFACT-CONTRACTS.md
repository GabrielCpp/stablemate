...
4. **Empty is not absent.** A present-but-unusable artifact (exists, parses, but yields an empty
   ...
   never need defensive "is it secretly empty?" checks (the Globex dispatcher now hard-fails on
...
The `qa-evidence` semantic rules are a port of the Globex workflow's
