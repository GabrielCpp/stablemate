...
```json
{
  ...
  "pid": 12345,
  "cmd": "go run ./tools/eventbridge-tail --event-bus acme-dev --port 7890",
...
```json
{
  ...
  "mechanism": "live",
  "cmd": "curl -s -X POST https://mobile-gateway.cm.example.com/auth/login ...",
  "exit_code": 0,
  "http_status": 200,
...
steps:
  ...
  - id: login
    ...
    cmd: >
      curl -s -w "\n%{http_code}"
      -X POST https://mobile-gateway.cm.example.com/auth/login
  ...
  - id: explicit_logout
    ...
    cmd: >
      curl -s -w "\n%{http_code}"
      -X POST https://mobile-gateway.cm.example.com/auth/logout
