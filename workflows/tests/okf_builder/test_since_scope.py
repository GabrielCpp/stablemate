...
coverage join measured only those. The per-citation `@digest` stamp carries a rebase for
free — a rebase leaves an unchanged file's stamp matching — and a teammate who never runs
stablemate cannot narrow against a diff that does not exist for them. The diff-scope
machinery is retired, and the four things this file used to assert are reduced to one —
that a retired input is still declared and unread.
"""
