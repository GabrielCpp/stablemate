...
* the command exists — `workhorse-hello-world` is a `[project.scripts]` row in
  `pyproject.toml` pointing at this module's `main`, and nothing else in this repo would
  notice its absence;
...
def test_the_documented_command_is_declared() -> None:
    """`workhorse-hello-world run --dry-run` is the documented command, so the script
    row is part of the example — not packaging trivia. Nothing else imports this
    package, so a dropped row is otherwise silent until a reader hits it."""
    pyproject = Path(__file__).resolve().parents[1] / "pyproject.toml"
    assert (
        'workhorse-hello-world = "workhorse_workflows.hello_world.workflow:main"'
        in pyproject.read_text()
    ), "hello-world declares no console script, so it has no command at all"
    assert callable(hello_world.main)


def test_the_documented_dry_run_walks_the_machine_green() -> None:
    """`workhorse-hello-world run --dry-run`, minus the CLI parsing.
...
if __name__ == "__main__":  # standalone, like every other test in this tree
    test_the_documented_command_is_declared()
