"""`load_survey_config` and `check_inventory` — the two nodes that decide where the survey's artifacts live and whether the granularity planner runs at all."""
...
def test_blank_parameters_fall_back_to_the_defaults(
    ...
    """`--params '{"rubric": ""}'` and an unset var reach the node the same way, which is why the script stripped each argument before defaulting it."""
    write(repo / "docs/survey/rubric.md", "# concern\n")
...
def test_a_missing_rubric_halts_the_run(repo: Path, logger: logging.Logger) -> None:
    """The rubric is the surveyor's only project-facing input: without it the planner and the assessors would hallucinate the concern being surveyed."""
...
def test_an_existing_inventory_freezes_the_enumeration(
    ...
    """A resume must consume the *same* list — a re-planned one would silently break the coverage claim, so the inventory outranks even a rules file sitting beside it."""
