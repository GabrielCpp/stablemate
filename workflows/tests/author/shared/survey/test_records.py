"""`validate_record` and `verify_records` — the two gates that let everything downstream trust the finding records."""
...
def test_a_record_for_another_unit_is_rejected(
    ...
    """The single most damaging way a record can be wrong: coverage would be claimed for a unit nobody looked at."""
...
def test_every_structural_error_in_a_finding_is_reported_together(
    ...
    """One rework turn, every problem — the fix loop is bounded, so a gate that reported one error at a time would burn the budget on plumbing."""
    write(
...
def test_a_blocked_record_must_name_its_gap(
    ...
    """`blocked` is never a bare shrug: the operator gate reads `openGaps`, so an empty one is the same as no answer."""
...
def test_no_inventory_means_nothing_was_surveyed(
    ...
    """The script's third `verify_ok` value."""
    result = verify_records(logger)
...
def test_a_pending_unit_is_never_waved_through(
    ...
    """The gate exists for exactly this: a unit re-pended by an operator (or never picked up) must send the loop back around instead of passing as covered."""
...
def test_an_invalid_record_is_reported_with_the_compact_wording(
    ...
    """`verify_records` carries its own copy of the ruleset, worded for one line of an operator's report rather than for the assessor being asked to fix it."""
    write_json(repo / INVENTORY, {"units": [_unit("src/api", "assessed")]})
    write(
        repo / FINDINGS / "src-api.md",
...
def test_an_accepted_disposition_closes_the_blocked_gap(
    ...
    """A blocked unit does not block the survey forever — but only a recorded, reasoned acceptance clears it, and the report still counts it as blocked."""
    write_json(repo / INVENTORY, {"units": [_unit("src/api", "blocked")]})
    write(
        repo / FINDINGS / "src-api.md",
        _record("src/api", "blocked", disposition="accepted"),
...
def test_a_split_is_lineage_not_a_drop(
    ...
    """`split_unit` legitimately removes a parent, and the children's paths are the proof — which is why this check reads paths and not just ids."""
...
def test_the_shrinkage_check_fails_open_with_no_committed_baseline(
    ...
    """Nothing committed, nothing to compare."""
    write_json(repo / INVENTORY, {"units": [_unit("src/api", "clean")]})
    write(repo / FINDINGS / "src-api.md", _record("src/api", "clean"))
