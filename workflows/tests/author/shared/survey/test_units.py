"""`select_next_unit` and `mark_unit` — the survey's loop, which keeps no state of its own: the inventory file and the finding records *are* the loop state."""
...
def test_done_and_blocked_units_are_both_skipped(
    ...
    """`assessed` and `clean` are the two done-states; `blocked` is set aside rather than retried, and stays an open gap for the coverage gate."""
    write_json(
        repo / INVENTORY,
        {
            "units": [
...
def test_an_empty_pending_set_is_the_coverage_proof(
    ...
    """Structural rather than post-hoc: the loop ends because nothing is left, not because a budget ran out."""
...
def test_a_missing_inventory_is_reported_not_raised(
    ...
    """The flow reads `has_unit` and moves to the coverage gate, where the missing inventory becomes a verify finding — the loop does not crash on it."""
    pick = select_next_unit(logger)
...
def test_a_unit_is_marked_with_its_records_own_status(
    ...
    """The record is the source of truth, not the node's parameters: an assessor that found nothing writes `clean`, and the inventory follows."""
...
def test_an_invalid_record_blocks_the_unit_without_overwriting_it(
    ...
    """Whatever the assessor did write is evidence for the operator, so the node marks the unit blocked and leaves the file alone."""
    write_json(repo / INVENTORY, {"units": [_unit("src/web")]})
...
def test_marking_a_unit_the_inventory_does_not_carry_is_reported(
    ...
    """A split parent, most likely — the status still comes back so the flow can log it, but `marked` is false and the run does not claim a write that did not happen."""
