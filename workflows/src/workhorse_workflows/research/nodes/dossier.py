"""The program dossier: what a program-level review is judged on, computed with zero model calls."""
...
def parse_status_table(
    ...
    """`(active_rows, superseded_rows)` from every `| Gate | ..."""
    unparsed = unparsed if unparsed is not None else []
    active: list[GateRow] = []
...
def build_dossier(
    ...
    """Read the program folder and compute the dossier."""
    repo = Path(repo_dir)
