...
3. **Resolve conflicts with judgment.** For each conflicted file (`git status`, `git diff --name-only --diff-filter=U`), understand *both* sides before choosing — preserve the intent of the base changes AND the epic's changes; never blindly take one side or delete code to make a conflict disappear. For generated files (OpenAPI/clients, mocks, codegen output), do **not** hand-merge — re-run the repository's codegen command and commit the regenerated result. Read the relevant layer instruction files for any code you touch, from those this repository installs: {{ instruction_refs("go", "go-testing", "react-router", "react-router-qa", "flutter", "flutter-testing", "pulumi") | default("(none installed — follow `AGENTS.md`)", true) }}. Conflict resolution must preserve the already-documented behavior from both branches; it may not invent or alter a service, screen, component, command, endpoint, flow, concept, format, or other observable contract. If preserving both sides requires a product/documentation decision, abort and report `failed` for operator/story-level resolution.
