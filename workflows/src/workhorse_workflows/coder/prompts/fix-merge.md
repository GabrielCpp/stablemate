...
3. **Resolve conflicts with judgment.** For each conflicted file (`git status`, `git diff --name-only --diff-filter=U`), understand *both* sides before choosing — preserve the intent of the base changes AND the epic's changes; never blindly take one side or delete code to make a conflict disappear. For generated files (OpenAPI/clients, mocks, codegen output), do **not** hand-merge — re-run the repository's codegen command and commit the regenerated result. Read the relevant layer instruction files for any code you touch, from those this repository installs: {{ find_by_tags("standards") | default("(none installed — follow `AGENTS.md`)", true) }}. Conflict resolution must preserve the already-documented behavior from both branches; it may not invent or alter a service, screen, component, command, endpoint, flow, concept, format, or other observable contract. If preserving both sides requires a product/documentation decision, abort and report `failed` for operator/story-level resolution.
...
5. **Commit on the epic branch.** Conclude the merge with a **Conventional Commit** subject — releases here are cut by release-please, which reads commit subjects and nothing else:
   `git commit --no-edit` (for the merge, whose generated subject release-please correctly ignores) — or if you staged manual resolutions, `git add -A && git commit -m "chore(<package>): merge {{ ci_base }} into feat/{{ ci_epic }}"`.
   `chore` is right for a conflict resolution: it releases nothing, because the behavior it preserves was already released by the commits on both sides. If resolving the conflict genuinely *changed* behavior, you are past what this stage may do — report `failed` instead.
