...
{% endif %}Bring **that** stack up (not whatever thin default is already running) and create the named fixtures/data before you exercise the path. **If the data/seed/migration/stored-procs the surface needs are absent, building or wiring them is IN SCOPE for this story — not a reason to skip.** A surface that renders blank "because there's no data" is the work, not a wall: seed it, add the migration, point at the capable profile, then exercise it. Do not walk away from a surface just because the story body didn't spell out the fixture. **Record the durable, code-independent part of the bring-up in the repo's `qa-stack.yml` manifest** (containers, emulators, the datastore and its baseline seed — its `prepare`/`seed` steps) so QA's `ensure_stack` step reuses exactly what you ran — never leave a stack backgrounded in your shell for QA to inherit; a process the workflow does not own dies at node teardown. **Do not put a `launch`/`health` entry in `qa-stack.yml` for a service pinned to this story's own working-tree source** (an API/dev server QA must run against *this* branch's code, not an adopted image) — `ensure_stack`'s health probe is always a `GET` expecting `200`–`399`, so a POST-only or GET-less surface can never satisfy it there. That service belongs in the QA plan's own `background()` step instead (`ready_cmd`/`ready_contains` when the readiness route is not a plain `GET`), which the QA plan authoring skill covers — record its bring-up command in the story's QA-plan notes, not in the stack manifest.
{% endif %}
