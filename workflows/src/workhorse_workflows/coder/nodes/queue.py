...
* **The legacy fallbacks stay too** — `docs/epics/epics-todo.json` in `select_epic` and
...
from workhorse_workflows.coder import paths, story_status
...
#: The epic queue, repo-relative — an ostler-managed OKF index whose front entry is next.
QUEUE_PATH = Path(paths.EPICS_INDEX)
...
#: The legacy JSON queue, kept as a fallback for repos and sandboxes with no doc graph.
LEGACY_QUEUE = Path("docs/epics/epics-todo.json")
...
def prune_epic(logger: logging.Logger, epic: str = "", todo_path: str = "") -> EpicPruned:
    ...
    return EpicPruned(pruned=_prune_json_sidecar(root / LEGACY_QUEUE, epic))
...
def _next_from_json(root: Path, epic: str, skip: set[str]) -> dict | None | str:
    ...
    for entry in stories:
        ...
        path = entry.get("path", "")
        story_md = (
            Path(path) if path else root / "docs" / "epics" / epic / "stories" / slug / "story.md"
        )
