...
def open_story_pr(
    ...
    Each PR carries the story's own heading as its title and the plan's summary as its
    body, plus the QA screenshots for a repo whose QA drives a browser or a device. The
    ...
    **`base_branch` reaches nothing, and that is preserved rather than repaired.** The
    ...
    title = _story_title(root, story_path) or f"Story: {story_slug}"
    ...
    for name in affected:
        ...
        _commit_in_repo(logger, repo_path, branch, title)
...
def _story_title(root: Path, story_path: str) -> str:
    """The story's first `# ` heading, which is the PR title."""
...
def _commit_in_repo(logger: logging.Logger, repo_path: Path, branch: str, title: str) -> None:
