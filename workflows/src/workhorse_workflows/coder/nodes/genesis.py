...
from workhorse_workflows.coder.contract import service_problems
...
@blueprint.node(stub=_stubs.classified)
...
@blueprint.node(stub=_stubs.built)
def init_skeleton(
    logger: logging.Logger,
    target_dir: str = "",
    service_root: str = "",
...
@blueprint.node(stub=_stubs.installed)
def install_farrier(
    logger: logging.Logger,
    target_dir: str = "",
    scaffolds: Sequence[str] = (),
...
@blueprint.node(stub=_stubs.valid)
def validate_genesis(
    ...
    * **`docs/epics/` exists.** ostler infers its graph profile from this directory, and
      only the `full` profile runs the structural doctor checks the author workflow's
      coverage gate depends on.
    * **`docs/backlog.md` exists.** `load_config` hard-fails without it.
