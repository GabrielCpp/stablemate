...
#: Directories that mean the same thing without any configuration at all.
MANAGED_DIRS = ("docs/epics", "docs/knowledge")
...
def detect_okf_docs(
    ...
    managed_tree = any(
        path.is_dir() for path in (requested, *(base / rel for rel in MANAGED_DIRS))
...
def verify_story_documentation(
    ...
    for change in packet.get("changedCode", []):
        ...
        grounded = (
            required.issubset(exactly_grounded)
