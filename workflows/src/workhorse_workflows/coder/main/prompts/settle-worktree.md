...
3. **Commit what belongs to this story, one commit per package.** Carry
   `Epic: {{ epic }}` and `Story: {{ story_id }}` as footers, spelled exactly so and
   nowhere else in the message — not bracketed into the subject.
