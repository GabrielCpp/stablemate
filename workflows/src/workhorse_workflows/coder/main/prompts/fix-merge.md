...
5. **Commit on the epic branch.** `git commit --no-edit` concludes the merge; commit staged manual resolutions as `chore(<package>): merge {{ ci_base }} into feat/{{ ci_epic }}` — a conflict resolution is `chore` because the behavior it preserves was already released by the commits on both sides. If resolving the conflict genuinely *changed* behavior, you are past what this stage may do — report `failed` instead.
