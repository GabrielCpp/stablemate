...
def read_runs(run: Run) -> list[dict[str, Any]]:
    ...
    for directory in sorted(p for p in runs_dir(run).glob("*") if p.is_dir()):
...
def churn_candidates(run: Run) -> list[dict[str, Any]]:
...
def hang_candidates(run: Run, threshold_s: float = 1800.0) -> list[dict[str, Any]]:
    ...
    **cap-wait** (sleeping on a usage cap until its window reopens). A node that took three
    ...
    """
    total, runs, longest = node_totals(runs_dir(run))
    cap = cap_wait_by_node(run.stage / "artifacts")
    containers = flow_containers(runs_dir(run))
    out: list[dict[str, Any]] = []
...
def phase_lines(phases: list[dict[str, Any]]) -> list[str]:
    ...
    for p in phases:
        mark = "✓" if p["rc"] == 0 else "✗"
        lines.append(f"  {mark} {str(p['phase']):<20}{minutes(float(p['wall_s'])):>8}  exit={p['rc']}")
    total = sum(float(p["wall_s"]) for p in phases)
    lines.append(f"    {'total':<20}{minutes(total):>8}")
    return lines
