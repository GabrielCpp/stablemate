...
- `code:` — where it's rendered, `path::symbol` (`groom/groom/assets/dashboard.js::RunRow`).
...
- `verify:` — the test that proves it (`test_tree_builder.py::test_paths_sharing_...`).
...
- `openapi:` (server) — link to the OpenAPI `format` node, when the server publishes
  one (omit when it opts out — e.g. groom's browser-facing routes set `include_in_schema=False`).
...
- **A `file` region** — `dashboard.html::the island mount points` — stays permitted
...
## 7. Worked examples (a groom-shaped service)

> These examples are **illustrative, and frozen**. They were written against an
> earlier groom — one with an Inbox mode, a `/changes` route, and a server that
> rendered HTML fragments — and they are kept in that shape because what they teach
> is the *profile*: which node types exist, which bullets they carry, how the
...
frames arrive over `/ws` and update the regions in place.
...
Serves the [groom shell](../gui/screens/groom.md) and pushes live updates over `/ws`.
The routes are browser-facing and webhook, not a public JSON API — every handler sets
