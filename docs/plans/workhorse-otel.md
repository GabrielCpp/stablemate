...
*(Design-time list. `groom/render.py` has since been removed: the dashboard talks JSON end to end,
so the telemetry pane is a Preact island in `groom/assets/dashboard.js` fed by `projection.py`.)*
