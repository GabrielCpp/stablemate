...
A `coder` run on Globex reached QA, which blocked with "web not reachable on :5173". The
...
boot is not sensitive at all — of Globex's five keys in §1, the two that actually define the
...
CREATE TABLE IF NOT EXISTS environments (
    ...
    project     TEXT,                       -- globex — inferred from the git repo, as today
...
```
<project>/<environment_id>/<KEY>        e.g.  globex/env-001/VITE_FIREBASE_API_KEY
...
saddlebag env set web-local VITE_FIREBASE_PROJECT_ID=globex              # → config
...
entries:
  ...
  - key: VITE_FIREBASE_API_KEY
    kind: secret          # value lives in the store, keyed globex/env-001/VITE_FIREBASE_API_KEY
...
4. **Workflow adoption.** The `setup-fix.md` rewrite, the `ensure_env` node, and the permission
   rule denying agent reads of `.env*`. Populate Globex's pool from `web/.env.example` as the
   first real consumer.
