...
branch. Validation failures are either routed through bounded automatic resolution or parked at an
operator context file; a terminal artifact or milestone failure commits an incomplete marker and
...
- does: otherwise routes reconciliation errors to the shared operator resolver
...
- does: otherwise routes integrity errors to graph resolution
...
- does: otherwise repeats integrity with an incremented resolution count
...
- does: otherwise routes milestone errors to the integrity resolver
...
- verify: visible(locator="operator-awaiting context", text="milestone")
- does: otherwise repeats milestone validation with an incremented resolution count
