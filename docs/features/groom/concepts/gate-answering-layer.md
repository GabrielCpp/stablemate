...
- consistency: `answer_gate` returns `AnswerResult(ok=False, message="unknown workspace volume for this container")` when `workspace_volume` is empty.
...
- consistency: after a successful answer-file write, the [workflow gate clearer](workflow-gate-clearer.md) removes a present in-memory gate entry for `(container_id, file_path)` from the selected [workflow container](workflow-container.md).
