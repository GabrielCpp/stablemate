...
`safe_relpath` is declared once in `groom/groom/docker_io.py`; it rejects empty, rooted, empty-segment, and parent-traversal paths, then returns the accepted segments joined with `/`. The source has no deprecation, wrapper, alternate implementation, or call-site preference that ranks the two documentation views — this concept states that fact rather than describing the symbol's behaviour itself, so it carries no `code:` citation of its own.
