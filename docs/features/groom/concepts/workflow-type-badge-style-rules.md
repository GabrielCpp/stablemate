...
stable `0`--`359` hue for any kind without requiring a palette entry. The badge renderer applies
that hue as a CSS custom property; these rules give it its appearance and reserve fixed colours
for the established `coder` and `author` kinds.
