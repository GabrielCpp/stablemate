...
Dashboard websocket send loop is groom's per-browser-tab outbound websocket pump and the detailed helper for the [Groom app module send-loop member](groom-app-module.md#method-send-loop). The [run dashboard websocket session](../http/groom.md#run-dashboard-websocket-session) invocation starts one loop with the accepted browser websocket and that tab's queue from the [dashboard client queue set](dashboard-client-queue-set.md); broadcasts and per-run pushes enqueue plain `dict` messages — a [dashboard state payload](../dashboard-state-payload.md), a per-run `detail`, a one-shot `notify`, or an `answered` confirmation — and this loop is the single place they become JSON text, one `json.dumps` per queued object, one websocket text frame for the [groom dashboard](../gui/screens/groom-dashboard.md). It is paired with the [dashboard websocket receive loop](dashboard-websocket-receive-loop.md), but it only owns server-to-browser delivery.
...
- payload type: `dict`; each queue item is JSON-serialized here and the result is the complete websocket text-frame body. Keeping serialization at the edge is why every producer upstream can stay in Python objects and why the socket and `GET /api/state` cannot drift into two encodings.
...
- item value: each `queue.get()` result is expected to be a JSON-serializable `dict`; no first-party validation, schema check, or `type` discrimination is performed before serializing it.
...
- step: Block until a broadcast or per-run push has queued one message object for this browser session.
- step: Serialize it to JSON and send it as one websocket text frame to the browser dashboard.
...
- Serializes the queued object with `json.dumps` and treats the result as the complete websocket text payload; no projection, validation, wrapping, or event classification happens in this layer.
- Sends that text on the supplied websocket with text-frame semantics.
