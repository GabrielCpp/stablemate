...
Dashboard websocket receive loop is groom's per-browser-tab inbound websocket pump and the detailed helper for the [Groom app module recv-loop member](groom-app-module.md#method-recv-loop). The [run dashboard websocket session](../http/groom.md#run-dashboard-websocket-session) invocation starts one loop with the accepted browser websocket; the loop receives decoded browser JSON messages from the [groom dashboard](../gui/screens/groom-dashboard.md) and delegates each message — together with that tab's own outbound queue — to the dashboard command handler that consumes [dashboard websocket command frames](../formats/dashboard-websocket-answer-frame.md). It is the inbound counterpart to the [dashboard websocket send loop](dashboard-websocket-send-loop.md): the receive loop owns browser-to-server frames, while the send loop owns queued server-to-browser messages.
...
- input-frame: one JSON value decoded from each inbound websocket message by the websocket layer; first-party dashboard code sends objects matching [dashboard websocket command frame](../formats/dashboard-websocket-answer-frame.md).
