...
Dashboard client queue set is groom's process-local fan-out target for browser dashboard websocket broadcasts. The [run dashboard websocket session](../http/groom.md#run-dashboard-websocket-session) invocation registers one outbound queue per connected tab, the [dashboard shell broadcaster](dashboard-shell-broadcaster.md), [sidecar blocked applier](sidecar-blocked-applier.md), and push/answer paths enqueue JSON message objects through [broadcast dashboard message](#method-broadcast-dashboard-message), and the [dashboard websocket send loop](dashboard-websocket-send-loop.md) serializes each queued object into a websocket text frame for the corresponding tab.

- code: groom/groom/state.py::CLIENTS
- refs: [run dashboard websocket session](../http/groom.md#run-dashboard-websocket-session), [dashboard websocket send loop](dashboard-websocket-send-loop.md), [dashboard shell broadcaster](dashboard-shell-broadcaster.md), [sidecar blocked applier](sidecar-blocked-applier.md), [dashboard state payload](../formats/dashboard-state-payload.md), [run watch registry](run-watch-registry.md)
...
- delivery model: best-effort in-process queueing; a successful broadcast means every queue present in the broadcast snapshot accepted the message, not that any browser has already received, parsed, or rendered it.
- ordering: messages are enqueued to each individual queue in caller order when callers await broadcasts sequentially; the set does not define a global client ordering.
...
- payload rule: the queue set accepts one already-projected `dict` and does not inspect its `type`, so a [dashboard state payload](../formats/dashboard-state-payload.md), a `notify`, and an `answered` message travel the same path. Serialization to JSON text happens later, in the send loop.
...
- excluded callers: `/api/state`, `/repos`, the file/diff/detail/traces panel endpoints, sidecar data-plane RPC replies, and sidecar reload commands do not register dashboard browser queues or enqueue dashboard-client messages through this set.
...
- meaning: one outbound message queue owned by one dashboard websocket connection.
...
- contents: the queue may be empty or already contain messages at registration time; registration does not inspect, drain, or seed it.
- initial snapshot: registration happens before the session sends the direct first `state` snapshot, so a concurrent broadcast after registration can enqueue to the queue before the send loop task begins consuming it.
...
- Bottoms out: the layer calls the built-in set discard operation for `CLIENTS` and the built-in `pop` on the `WATCHING` map; it calls no other first-party groom symbol.

### method-broadcast-dashboard-message
...
- message: required `dict`; default none; carries its own `type` discriminator — `state` for the whole fleet, `notify` for a one-shot alert, `answered` for an answer confirmation.
...
- target type: every snapshot member is treated as an awaitable queue with a `put` method accepting the supplied object.
...
- step: Receive one already-projected JSON message object from a caller that owns projection and payload composition.
- step: Copy the current client set into a list to freeze the target queues for this broadcast pass.
- step: For each queue in that target list, await one `put` of the exact supplied object.
- step: Complete after the last target queue accepts the message, or propagate the first queue/cancellation exception encountered after any earlier successful puts.
...
- Broadcast cancellation or queue-put failure can produce partial delivery to the target list; already-enqueued messages remain queued, later target queues may not receive the message, and the client set itself remains unchanged. A tab that missed a broadcast recovers through the [dashboard resync poller](dashboard-resync-poller.md), not through anything this set does.
