...
- Does not: inspect Docker, decide whether pruning is safe during a Docker outage, render or broadcast dashboard messages, answer or mutate gate files, restart containers, write logs, persist state, or mutate the caller's `present_ids` set.
