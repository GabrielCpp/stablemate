...
- serialization: every queued frame is encoded as ordinary JSON text matching the [sidecar websocket frame](../formats/sidecar-websocket-frame.md) contract; the sender does not validate protocol fields before serialization.
...
- lifecycle: the containing session starts one sender task after starting the watch task and cancels it during session cleanup.
