...
- consistency: dashboard-client-store — selecting a run writes its `selected` and `detail` fields in the same `store.set` call, so a render never pairs a new selection with the previous run's pane.
