...
- name: visible text from the optional workflow type badge followed by the repository option label generated from workflow/container name and checkout directory, for example `coder coder-001/globex` when a `coder` badge is rendered or `author-002` when no checkout directory and no badge are present.
