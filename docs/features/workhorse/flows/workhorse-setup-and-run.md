...
distribution that declares its own [`workhorse-<name>`](../formats/workflow-format.md#console-script)
...
- steps:
  1. **Install a workflow distribution** — the bundled `workhorse-workflows`, a
     ...
     [console script](../formats/workflow-format.md#console-script) that becomes the command, and
  ...
  2. **[`workhorse-<name> run [<flow>]`](../workhorse.md#run)** — the command *is* the
     ...
     [`Registry`](../formats/workflow-format.md#registry) straight to `run`, which calls
     [`Registry.directory()`](../formats/workflow-format.md#registry) eagerly, so a zip-imported
  ...
  3. **The driver walks the machine** — `run_pyflow` seeds or resumes the one stable run dir
     ...
     [`Workflow`](../formats/workflow-format.md#workflow-subclass) from `--params`, and hands it to
...
- end: the process exits `0` (the entry flow returned
  [`Done`](../formats/workflow-format.md#transition)), `1` (a `PyflowError` — an explicit
