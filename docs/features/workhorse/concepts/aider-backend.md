...
so it drives OpenRouter models directly (e.g. `openrouter/example-org/example-model`) with no
proxy — the OpenRouter provider pin and prompt caching live in aider's own model-settings file,
not here.

`runner/backends/aider.py` holds both the class and the plain-text turn loop it needs
([`_run_text_turn`](run-text-turn.md)). That loop is *not* beside
[`stream_jsonl`](stream-jsonl.md) in the shared modules, deliberately: aider is its only user, and
a shared module holding one implementation's mechanics is the shape the port package must not
have.

- code: `workhorse/workhorse/runner/backends/aider.py::AiderBackend`
...
- verify: `workhorse/tests/test_backends.py::test_aider_run_turn_builds_noninteractive_cmd`,
  `workhorse/tests/test_backends.py::test_aider_effort_clamped_to_high`,
  `workhorse/tests/test_backends.py::test_aider_no_effort_omits_flag`,
...
- **`run_turn(prompt, node_id, session_id_path, model=None, *, timeout, resilience, cwd=None,
  add_dirs=None, effort=None)`** — `timeout` and `resilience` are keyword-only and required, like
  every other backend ([why](agent-backend.md#run_turn-abstract)). Builds the argv:
  ```
  ...
  - `add_dirs` has no aider equivalent — aider always works the repo at `cwd` — and is silently
    ignored.
  - Delegates the spawn and classification to
    [`_run_text_turn("aider", cmd, node_id, timeout, cwd, session_id_path, resilience=resilience,
    env_extra=self.harness_env())`](run-text-turn.md): the whole stdout transcript becomes the
    turn's result text and, doubling as the diagnostics channel, is what
    [`finalize_turn`](finalize-turn.md) scans for overflow/transient markers. Raises
    [`BackendInvocationError`](classify-turn.md#backendinvocationerror) on failure, exactly as
    classified there.
  - `env_extra` is [`harness_env()`](agent-backend.md#harness_env-concrete) — the operator's
    `[harness.aider].env` table, read per turn and layered over the inherited environment.
- **`compact(session_id_path, node_id, model=None, *, timeout, resilience)`** — always returns
  `False`: aider has no resumable session to compact (each turn is a fresh `--message`), so the
  [resilience ladder](run-agent.md) reframes on overflow instead.

### `_aider_effort`

- code: `workhorse/workhorse/runner/backends/aider.py::_aider_effort`
...
- [`_run_text_turn`](run-text-turn.md) — the plain-text turn runner `run_turn` delegates to; owns
  the process spawn, live-echo accumulation, transcript-derived usage, and hand-off to
  [`finalize_turn`](finalize-turn.md).
- [`get_backend`](get-backend.md) — resolves `"aider"` to a cached `AiderBackend()` instance.
- [`AgentBackend`](agent-backend.md) — the port this class implements; its `harness_env` is the one
  method aider inherits rather than overrides.
