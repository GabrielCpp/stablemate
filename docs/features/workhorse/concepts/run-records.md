...
- type: `str`
- required: true
- semantics: retired engine's current node identifier; its presence distinguishes this shape from a pyflow checkpoint
...
- type: `BaseModel`
- semantics: point-in-time Git observation embedded in a run record; empty values mean the fact was not observed
