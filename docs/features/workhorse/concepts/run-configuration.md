...
no agent uses `NullBackend`, not a nullable collaborator.
...
- default: `7200.0`
- required: true
- semantics: single cap wait interval in seconds before probing whether the cap cleared early; zero disables probing
...
- default: `NullBackend()`
- required: true
- semantics: already-resolved agent backend; absence is represented by the null implementation rather than `None`
...
- required: true
- semantics: maximum captured transcript size per turn; captures reaching it are truncated with a marker
