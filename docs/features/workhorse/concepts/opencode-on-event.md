...
shared JSONL loop that does — `stream_jsonl` itself is vocabulary-agnostic and just calls
...
- **Input:** `(event: dict, state: dict, node_id: str, diagnostics: list)`, matching
  ...
  - `state` — the turn's accumulator, starting as `{"result_text": "", "session_id": None}`;
    ...
    below) — not part of `stream_jsonl`'s own contract, internal to this adapter only.
...
The `diagnostics` append feeds [`stream_jsonl`](stream-jsonl.md#early-abort--stop-the-clis-own-retry-loop)'s
per-line cap-abort scan (run by its caller immediately after `on_event` returns) and, at the end of
the stream, [`finalize_turn`](finalize-turn.md)'s classification — an opencode structured error
...
`stream_jsonl`'s JSON-decode-fails branch instead, per `test_opencode_cap_log_line_aborts_stream_early`;
...
- `OpenCodeBackend.run_turn` (`workhorse/workhorse/runner/backends.py::OpenCodeBackend`) — the sole
  caller, passing `_opencode_on_event` to `stream_jsonl(cmd, node_id, timeout, None,
