...
2. **Inputs** — the data the flow already validated, inlined verbatim (plan text, backlog
   ...
   defect that loops the producing turn, so the prompt carries no {% raw %}`{% if %}`{% endraw %} fallback
...
- **{% raw %}`{% if mode %}`{% endraw %} variables** hide two prompts inside one file, and every reader pays
