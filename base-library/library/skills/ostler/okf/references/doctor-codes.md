...
| `unarranged-request-body` | error | An obligation's route needs a request body (a method other than `GET`/`DELETE`/`HEAD`/`OPTIONS`) and the book has no grammar yet to arrange one — so the call is withheld rather than sent with a fabricated body that would draw a false failure from the app. Raised from `Gap`s via `doctor.gap_findings`, against one compiled plan and its context packet. |
