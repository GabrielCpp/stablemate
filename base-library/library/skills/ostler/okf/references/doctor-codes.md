...
| `unparsable-bullet-value` | error | A bullet whose key declares a `value_kind` (`screen.route`/`endpoint.path` — `route`; `screen.entry` — `door`; `server.entry-url`/`runbook.entry-url` — `url`; `endpoint.method` — `http-method`) carries a value its kind's parser rejects — a role (`required=True`) checks presence only, not what the value may say, so `- method: fetch-data` or a prose `entry: no; it is reached from …` cleared every other check. The parser is the one a consumer already runs (`ostler.routes.literal_route`, `ostler.reach.is_route`, …), never a new grammar. An empty value is *absent*, not malformed, and is `missing-required-bullet`'s to find. Fix the bullet to spell a value of the declared kind, or delete it where the key is not required. |
...
| Code | Sev | Trigger and remedy |
| --- | --- | --- |
| `conflicting-surface-driver` | error | Two or more runbooks name this surface (via `surfaces:`) and state different `driver:` values for it. Every check that needs a driver for this surface — the route grammar reachability is computed in, the value-kind grammar a node's bullets are held to — has no single answer until the runbooks agree, and both degrade to treating the surface as undeclared rather than guess. Read the runbooks and settle which `driver:` is right. A surface is the feature directory, so two drivers cannot be held for one: if both are correct, only the runbook whose driver this surface is exercised with declares `surfaces:` into it (the key is optional), and if the two are separate services they belong under two feature directories. QA reports it as well: `qa compile-plan` gaps every obligation on the surface under this code rather than under `uncompilable-claim`, because the book states two drivers, not none. |
