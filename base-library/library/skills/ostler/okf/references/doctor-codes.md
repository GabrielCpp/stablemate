...
| `unstamped-citation` | warn | A local `code:` target carries no `@digest` yet — the migration-in-flight case, before `ostler stamp --from-catalog` has run on this book. It is stamped by that migration or when a turn that edits this node commits; never hand-write the digest. |
| `unreachable-citation` | warn | A repository-qualified `code:` target carries no `@digest` and names a repository other than the book's own — `ostler stamp` does not resolve a foreign checkout yet, so this citation cannot earn one here. Nothing to do on this book alone; it is stamped once foreign-checkout resolution lands. |
