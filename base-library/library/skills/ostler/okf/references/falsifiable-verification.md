...
Read this when you are writing or backfilling `verify:` bullets in bulk, repairing an
`undeclared-obligation`, `weak-check` or `unstated-precondition` finding, or reviewing checks
somebody else declared.

For the vocabulary itself — the calls and their arguments — see
[check-vocabulary.md](check-vocabulary.md); for where the bullet goes and what binds it to a
claim, [`../SKILL.md`](../SKILL.md) and [bullet-grammar.md](bullet-grammar.md). This page is only
about whether what you declared could ever fail.
