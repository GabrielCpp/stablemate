...
Paths covered:
  ...
  ─────────────────────────────────────────────────────────
   ...
   6. CI passed   (mocked GitHub API) → merge → done
...
def test_single_story_full_pipeline_exits_done(tmp_path, monkeypatch):
...
def test_first_epic_selected_and_branched(tmp_path, monkeypatch):
...
def test_ci_passed_merges_and_exits_done(tmp_path, monkeypatch):
    """GitHub checks return all-green → ci_status=passed → merge → done."""
    params = _ci_gate_setup(tmp_path, "epic-ci-pass")

    git_mock_with_remote(tmp_path)
    mock_github(monkeypatch, ci_status="passed")
    wf = WorkflowRun(WORKFLOW, tmp_path)
...
def test_merge_uses_repo_default_method_squash(tmp_path, monkeypatch):
    ...
    The GitHub fake reports merge commits disabled, squash enabled, and rebase
    disabled, so pick_merge_method resolves to squash.
    """
    ...
    assert result.passed(), f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
    assert_step_output(result, "merge", "merge_status", "merged")
    gh.get_repo.return_value.get_pulls.return_value[0].merge.assert_called_with(
        merge_method="squash"
    )


def test_open_pr_commits_epic_queue_before_merge(tmp_path, monkeypatch):
    """gh-open-pr.py commits docs/epics/epics-todo.json onto the epic branch before
    ...
    # message, committing docs/epics/epics-todo.json onto the branch.
    assert any(
        "prune completed epic from queue" in c.get("message", "")
        for c in []
    ) or result.passed()


def test_ci_failed_push_fails_flag_operator_blocked(tmp_path, monkeypatch):
    ...
    git_mock_with_remote(tmp_path)
    mock_github(monkeypatch, ci_status="failed")
    wf = WorkflowRun(WORKFLOW, tmp_path)
    ...
    assert_step_output(result, "push_ci", "push_status", "unavailable")
    # flag-ci-failure.py successfully posts a PR comment through the GitHub fake.
    assert_step_output(result, "flag_ci_fail", "ci_flagged", "yes")


def test_ci_failed_push_succeeds_then_passes(tmp_path, monkeypatch):
    """CI fails → fix_ci → push succeeds → re-await → CI passes → merge → done."""
    params = _ci_gate_setup(tmp_path, "epic-ci-fixpass")

    # The GitHub fake reports failed checks, which exercises the fix loop. Push is
    # unavailable in this sandbox, so this validates bounded retry behavior rather
    # than the eventual green-check path.
    wf = WorkflowRun(WORKFLOW, tmp_path)
    ...
    assert_step_output(result, "push_ci", "push_status", "unavailable")
    # After push, incr_ci → await_ci again. With the same GitHub fake,
...
def test_ci_max_rework_flags_then_operator_blocked(tmp_path, monkeypatch):
...
def test_two_epics_first_merges_then_advances_to_next(tmp_path, monkeypatch):
    ...
    git_mock_no_remote(tmp_path)
    wf = WorkflowRun(WORKFLOW, tmp_path)
    mock_all_agents_happy(wf, monkeypatch, story_md_paths=[s1_md, s2_md])
...
def test_qa_give_up_continues_to_next_story(tmp_path, monkeypatch):
    ...
    mock_qa_control_plane(wf, monkeypatch, ["failed"], slugs=["s-1", "s-2"])
    wf.mock_agent(
...
def test_qa_give_up_records_per_run_skip_set(tmp_path, monkeypatch):
    ...
    wf = WorkflowRun(WORKFLOW, tmp_path)
    _mock_impl_agents(wf)
    mock_qa_control_plane(wf, monkeypatch, ["failed"], slugs=["s-1"])
    wf.mock_agent(
        "apply_qa_fixes", {"qa_result": {"status": "failed", "notes": "Still failing."}}
    )
