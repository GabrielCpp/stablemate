...
through ostler's markdown parser, which is why it is **the** path. The body-only rewrite below
is the fallback of last resort, for the one case where the doc graph can't resolve the slug at
all — a story.md with no frontmatter, or a non-standard layout (the workflow test sandboxes are
exactly this). It locates the field with ostler's parser too, so the two paths write the same
line. There is no ostler-is-missing path: the workflow declares ``dist: ostler`` in its
``requires:`` block, so an interpreter that cannot import it never reaches node one.
