...
A new repository has just been created at `/tmp/todo-app`. Git is
...
- The service skeleton at `api`, exactly as the native
...
- **Do not create the skeleton yourself.** You are extending one the stack's own init tool
  already produced. If `api` looks empty or lacks its
