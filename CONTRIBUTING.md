...
clone has `core.hooksPath` unset, so the two commit guards are silently off until
something sets it — and the commit that needed stopping is usually the first one.
