...
(`projects/globex/globex-developer`), so two projects can both ship a
...
def _overlay_source(tmp_path: Path) -> Source:
    ...
    skill_file.write_text(
        textwrap.dedent(
            """\
            ---
            name: globex-developer
            description: "Globex developer workflow."
            ---

            # Globex Developer Workflow
            """
        ),
        encoding="utf-8",
    )
    return Source(
        kind="skill",
        path=skill_file,
        rel="projects/globex/globex-developer/SKILL.md",
        id="projects/globex/globex-developer",
    )


def _renderer(tmp_path: Path, source: Source) -> Renderer:
    return Renderer(tmp_path / "repo", "globex", {}, {}, [source], [])
...
def test_exact_prefixed_name_still_resolves(tmp_path):
    source = _overlay_source(tmp_path)
    renderer = _renderer(tmp_path, source)
    assert renderer.optional_skill_source("globex-developer") is source
