...
def test_aggregated_agents_file_carries_no_banner_at_all(tmp_path):
    # Only Claude strips HTML comments before loading, so a "generated — do not
    # edit" line survives into a codex session as an *instruction* in an
    # always-loaded rules file, where it reads as governing the repo rather than
    # the file. The banner rides on the CLAUDE.md pointer, which Claude strips.
    source = _overlay_skill(tmp_path, "go-qa", "# Go QA\nRules.\n")
    renderer = _renderer_with(tmp_path, skills=[source])
    target_dir = tmp_path / "svc"
    target_dir.mkdir()
