...
def test_claude_pointer_gets_the_html_comment_banner(tmp_path):
    ...
    # The body is not duplicated here: this file only points at the real one.
...
def test_aggregated_agents_file_carries_only_the_one_line_marker(tmp_path):
    # Every harness reads AGENTS.md and only Claude strips HTML comments, so the
    # multi-line banner would be installer trivia in a codex session's context.
    source = _overlay_skill(tmp_path, "go-qa", "# Go QA\nRules.\n")
    renderer = _renderer_with(tmp_path, skills=[source])
    target_dir = tmp_path / "svc"
    target_dir.mkdir()
    ...
    marker, _, body = content.partition("\n")
    assert marker.startswith("<!-- generated by farrier")
    assert "`farrier source AGENTS.md`" in marker
    assert "<!--" not in body
    assert "# Go QA" in body
