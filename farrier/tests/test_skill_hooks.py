...
def test_a_skill_declaring_nothing_declares_no_hooks(tmp_path):
    """The common case, and the one that must stay free: a bundled `scripts/` helper is something the *agent* runs, and installing it into every commit would be a surprise nobody asked for and nobody could see coming."""
...
def test_install_never_raises_on_a_malformed_declaration(tmp_path):
    """A repo pinned to an older base library must keep installing; one bad key in a skill it does not even select cannot be what stops it."""
...
def test_a_prompt_cannot_declare_hooks(tmp_path):
    """A prompt bundles no scripts, so there is nothing to install and run — and a declaration that can never work should fail where it was written."""
