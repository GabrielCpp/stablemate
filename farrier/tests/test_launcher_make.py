...
def test_the_launch_creates_a_worktree_root_for_this_run_alone(launcher: Path):
    """Two launches must not land in one directory, so the run id is in the path — and the launcher, not the container, is what creates it (the container cannot make a host directory that is not already bound)."""
...
def test_a_hyphenated_workflow_name_survives_the_eval(launcher: Path):
    """`$(eval)` re-parses its argument as makefile text, which is exactly where a name with a hyphen or an unlucky character would come apart."""
...
def test_a_workflow_nobody_installed_fails_instead_of_launching_nothing(launcher: Path):
    """The reason these are generated targets rather than one `agent-run-%` pattern rule: a pattern rule matches anything and would happily launch a container for a workflow that does not exist."""
...
def test_an_unrelated_target_never_pays_for_discovery(launcher: Path):
    """Discovery shells out to pipx, and this file is included by the repo's root Makefile — so every `make <anything>` would pay for it if it were unconditional."""
...
def test_a_profile_reaches_the_container_with_a_config_to_look_it_up_in(launcher: Path):
    """A profile is a name; without the file it names it resolves to nothing and the run takes the harness defaults instead."""
...
def test_a_launch_that_names_no_profile_selects_nothing(launcher: Path):
    """The behavior every run had before profiles existed: models resolve from the config's top-level tables, and an empty AGENT_PROFILE adds no flag."""
    result = _make(launcher, "-n", "agent-run-coder")
    assert result.returncode == 0, result.stderr
...
def test_a_config_that_cannot_be_read_stops_the_launch(launcher: Path):
    """Falling back to the machine's config would run a CI or benchmark launch on models nobody chose — and report success."""
