...
def local_instruction_marker(dest_name: str) -> str:
    """The one-line generated marker at the top of an aggregated AGENTS.md.

    Every harness reads AGENTS.md, and only Claude strips block-level HTML
    comments before loading — so the full banner would be a dozen lines of
    installer trivia in a codex or copilot session's context on every turn. One
    line is the smallest thing that still stops someone editing a generated file
    by hand, and it names the command that finds the real source.
    """
    return (
        f"<!-- generated by farrier — do not edit; sources: `farrier source {dest_name}` -->\n\n"
    )
...
class Renderer:
    ...
    def render_local_instruction(
        ...
        The file carries a one-line generated marker rather than the full
        provenance banner: every harness reads AGENTS.md, and only Claude strips
        block-level HTML comments before loading, so a banner here is context
        the others pay for on every turn. The banner lives in the CLAUDE.md
        ...
        for source, is_prompt in selected:
            ...
            if part:
                parts.append(part)
        rendered = local_instruction_marker(output_path.name) + "\n\n---\n\n".join(parts)
