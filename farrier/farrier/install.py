...
# prompts render against at run time. It uses your Claude subscription (seeded from
# ~/.claude/.credentials.json) and persists the agent's work, sessions, and run
# artifacts (under the repo-local .agents/runs).
#
...
# `make agent-install`). workhorse also auto-detects this path from AGENT_REPO_DIR.
CONTEXT_FILE ?= $(CURDIR)/.agents/agents-context.json
...
class Renderer:
    ...
    def context_manifest(self, target: str) -> dict[str, Any]:
        ...
        """
        skill_key = "copilot-instruction" if target == "copilot" else target

        def rel(path: Path) -> str:
            return path.relative_to(self.repo).as_posix()

        instructions = {
            key: rel(self.skill_output_path(source.id, skill_key))
