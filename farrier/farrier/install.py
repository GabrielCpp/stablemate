...
def _prepend_workflow_header(content: str, source: Path, rel_in_workflow: str) -> str:
    ...
    header = (
        ...
        f"# Canonical source: `farrier config show | grep library_dir` → workflows/{rel_in_workflow}\n"
        f"# Regenerate      : farrier --repo .\n"
    )
...
Edits made here will be **overwritten** the next time `farrier --repo .` runs.
...
class Renderer:
    ...
    def skill_dir_path(self, target: str) -> Path:
        if target == "copilot":
            return self.repo / ".github" / "instructions"
        if target == "codex":
    ...
    def render(
        ...
        for workflow in workflows:
            ...
            for source in sorted(path for path in root.rglob("*") if path.is_file()):
                if should_skip_workflow_file(source, root):
                    continue
                rel = source.relative_to(WORKFLOWS)
                output_path = self.repo / ".agents" / "workflows" / rel
                content = source.read_text(encoding="utf-8")
                # Render Jinja2 templates in workflow prompts
                if source.parent.name == "prompts" and source.suffix == ".md":
                    content = self.render_templates(content, "claude", output_path)
                else:
                    content = _prepend_workflow_header(content, source, rel.as_posix())
                outputs[output_path] = content

            # Generate a CLAUDE.md in the workflow directory explaining it is
            # farrier-generated and how to locate and edit the canonical source.
            claude_md_path = self.repo / ".agents" / "workflows" / workflow / "CLAUDE.md"
            outputs[claude_md_path] = _workflow_claude_md(workflow)

        # Launcher scaffolding: emitted exactly once when >= 1 workflow installs.
        if workflows:
