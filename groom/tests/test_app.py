...
def test_repos_endpoint_lists_one_entry_per_container_repo():
    ...
    client = _hermetic_client()
    try:
        with patch.object(groom_app.docker_io, "list_repo_dirs", return_value=["globex", "api-service"]):
    ...
    assert 'data-label="coder-001/globex"' in body and 'data-label="coder-001/api-service"' in body
...
def test_files_endpoint_returns_newline_separated_paths():
    ...
    try:
        with patch.object(groom_app.docker_io, "list_files", return_value=["README.md", "src/a.py"]) as lf:
            resp = client.get("/files/abc123", params={"repo": "globex"})
    finally:
        client.__exit__(None, None, None)

    assert resp.text == "README.md\nsrc/a.py"
    assert lf.call_args[0] == ("ws-vol", "globex")
...
def test_file_endpoint_joins_repo_and_path_and_returns_content():
    ...
    try:
        with patch.object(groom_app.docker_io, "read_file", return_value="print(1)\n") as rf:
            resp = client.get("/file/abc123", params={"repo": "globex", "path": "src/a.py"})
    finally:
        client.__exit__(None, None, None)

    assert resp.text == "print(1)\n"
    assert rf.call_args[0] == ("ws-vol", "globex/src/a.py")
...
def test_file_endpoint_swallows_unsafe_path():
    ...
    try:
        ...
        with patch.object(groom_app.docker_io, "read_file", side_effect=ValueError("unsafe")):
            resp = client.get("/file/abc123", params={"repo": "globex", "path": "../../etc/passwd"})
    finally:
        client.__exit__(None, None, None)
...
def test_diff_endpoint_passes_repo_through():
    ...
    try:
        with patch.object(groom_app.docker_io, "git_diff", return_value="diff --git a/x b/x\n") as gd:
            resp = client.get("/diff/abc123", params={"repo": "globex"})
    finally:
        client.__exit__(None, None, None)

    assert resp.text == "diff --git a/x b/x\n"
    assert gd.call_args[0] == ("ws-vol", "globex")
...
def test_file_content_prefers_sidecar_socket():
    ...
    try:
        with patch.object(groom_app.docker_io, "read_file") as rf:
            resp = client.get("/file/abc123", params={"repo": "globex", "path": "a.py"})
    finally:
        client.__exit__(None, None, None)
...
def test_apply_hello_marks_blocked_with_gate():
    ...
    _run_apply_hello(
        "abc123def456",
        {
            "identity": {"container_id": "abc123def456", "name": "coder-1", "repo_name": "Globex", "repo_branch": "main"},
