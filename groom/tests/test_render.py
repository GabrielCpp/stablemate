...
def test_tree_groups_workers_by_repo_and_badges_type():
    wfs = [
        _wf("a", repo_name="globex", repo_branch="main", workflow_type="coder", state=WorkflowState.RUNNING),
        _wf("b", repo_name="globex", repo_branch="main", workflow_type="author", state=WorkflowState.IDLE),
        _wf("c", repo_name="api-service", repo_branch="dev", workflow_type="coder", state=WorkflowState.RUNNING),
    ]
...
def test_changes_groups_diffs_per_repo():
    wfs = [
        _wf("a", repo_name="globex", repo_branch="main", state=WorkflowState.RUNNING),
        _wf("b", repo_name="globex", repo_branch="main", state=WorkflowState.IDLE),
        _wf("c", repo_name="api-service", repo_branch="dev", state=WorkflowState.RUNNING),
    ]
    html = render.render_changes(wfs)
    assert html.count('class="chg-repo"') == 2       # two repo groups
    assert "globex@main" in html and "api-service@dev" in html
