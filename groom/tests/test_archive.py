"""Archival: telemetry leaving SQLite for a file, and prune's refusal to delete anything that has not gone."""
...
def test_a_run_still_emitting_is_held_whole_rather_than_half_archived():
    """A run whose oldest rows have expired but which is still writing would be archived with a hole in it: the file would end at the cutoff and the rows after it would have nowhere to go."""
    with _DB():
...
def test_scratch_runs_are_never_archived():
    """A suite's mkdtemp run dir is junk by construction; prune deletes it without an archive, so archiving it forever would be the bug."""
    with _DB():
...
def test_every_record_carries_the_keys_a_transcript_joins_on():
    """The point of the file is that it sits beside the transcripts and shares their index: run, node, and for a span the visit's generation and seq."""
    with _DB():
        _seed()
        _live_dir()
        archive.sweep(retention_days=30, now=NOW)
...
def test_the_liveness_and_activity_gauges_never_reach_the_file():
    """They describe where a live groom thinks a run is."""
    with _DB():
        _seed()
        _live_dir()
        archive.sweep(retention_days=30, now=NOW)
...
def test_a_write_that_fails_leaves_the_run_in_the_database(monkeypatch: pytest.MonkeyPatch):
    """Fail-closed is the whole contract: the delete is reachable only through a successful write and a successful move, so a broken archiver costs disk, never data."""
    with _DB():
        _seed()
        _live_dir()
        def _disk_full(
...
def test_an_interrupted_sweep_finishes_instead_of_rewriting_or_colliding():
    """Crash between the move and the delete: the file and the directory are in place and the rows are still there."""
    with _DB():
        _seed()
        _live_dir()
...
def test_a_frozen_run_is_never_overwritten_by_a_later_run_of_the_same_name():
    """An archived run is read-only forever."""
    with _DB():
        _seed("R1", ts=OLD)
        _live_dir()
        archive.sweep(retention_days=30, now=NOW)
...
def test_a_bounded_pass_takes_the_oldest_runs_and_reports_the_rest_pending():
    """A first sweep over a year of backlog must not hold the writer for an hour, so the pass is bounded and the remainder is simply next time's work."""
    with _DB():
...
def test_prune_refuses_to_delete_a_run_no_archive_holds():
    """Retention used to be a data-loss dial, which is why it had to be set to a year."""
    with _DB():
        _seed()
...
def test_prune_leaves_a_run_inside_the_window_alone_even_when_archived():
    """`archived` says a run *may* go, not that it must: the age test still decides, so a run archived early is not deleted early."""
    with _DB():
...
def test_what_is_archived_is_read_from_the_directory_not_an_index():
    """There is no index table, deliberately: a second source of truth about what is on disk is a second thing that can be wrong."""
    with _DB():
        _seed()
        _live_dir()
        archive.sweep(retention_days=30, now=NOW)
...
def test_a_transcript_record_resolves_under_either_root():
    """The index row still says ``<run_id>/`` after archival, because the move is the only thing that happened."""
    with _DB():
        _seed()
        live = _live_dir()
        (live / "001-00003-plan__s1").mkdir()
