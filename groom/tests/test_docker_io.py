...
def test_find_repo_dir_extracts_parent_of_dot_git():
    with patch.object(
        docker_io.subprocess, "run", return_value=_completed(stdout="/vol/Globex/.git\n")
    ):
        assert docker_io.find_repo_dir("vol-1") == "Globex"
...
def test_git_diff_returns_stdout_on_success():
    ...
    with patch.object(docker_io, "find_repo_dir", return_value="Globex"), \
...
def test_list_files_returns_repo_relative_paths_and_prunes_vendor_dirs():
    ...
    def _fake_run(args, **kwargs):
        captured["args"] = args
        return _completed(stdout="/vol/Globex/src/a.py\n/vol/Globex/README.md\n")

    with patch.object(docker_io.subprocess, "run", _fake_run):
        paths = docker_io.list_files("workhorse_workspace", "Globex")

    # Paths are returned relative to the repo dir (not the volume) and sorted.
    assert paths == ["README.md", "src/a.py"]
    assert "find" in captured["args"] and "/vol/Globex" in captured["args"]
    assert "-prune" in captured["args"]
    for skip in docker_io._SKIP_DIRS:
        assert skip in captured["args"]
...
def test_list_files_empty_on_docker_failure():
    with patch.object(docker_io.subprocess, "run", return_value=_completed(returncode=1)):
        assert docker_io.list_files("vol-1", "Globex") == []
