---
name: stablemate-farrier-agent-library
...
Cross-link sibling skills with the `{{ instruction_file("<name>") }}`
...
```
{{ find_by_tags("web", "tests") }}    → "however this repo writes web tests"
```
