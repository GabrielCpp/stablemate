---
name: stablemate-farrier-skills-writing
...
every run. Its neighbours own the other two halves: [[ostler-repo-docs]] decides *which surface*
material belongs on (root `CLAUDE.md`, nested, README, `docs/`), and [[farrier-agent-library]] owns
...
- **Which surface the material belongs on** — root vs nested `CLAUDE.md`, README, `docs/` →
  [[ostler-repo-docs]]. That skill decides *where*; this one decides *how it is written*.
- **Library plumbing** — adding a skill to a pack, `agents.yml` selection, re-installing the
  generated `.claude/` and `.github/` copies → [[farrier-agent-library]]. Those copies carry a
