...
(`ladder.py` is the recovery ladder over `failure.py` / `process.py` / `caps.py` /
`reframe.py` / `extract.py`, and `backends/` the agent-CLI port and one adapter
...
- **Tests go in `tests/test_<area>.py`** and must be dependency-free and standalone: each
  ...
  `_run_turn_with_recovery`) and patching sleeping so nothing hits the network or waits
  in real time. `make test` runs them all. Add or extend a test for any behavior change.
