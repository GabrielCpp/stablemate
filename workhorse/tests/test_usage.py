...
def test_claude_result_keeps_its_own_key_names():
    """Claude's spelling IS the canonical spelling — the store already holds spans using it, so normalize must be the identity on these fields."""
...
def test_unknown_shape_yields_nothing_rather_than_raising():
    """The copilot shape could not be verified, so the extractor must degrade to a missing attribute — never an exception on the hot path of every event."""
...
def test_booleans_are_not_token_counts():
    """isinstance(True, int) is True in Python; a stray flag must not land in the store as a count of 1."""
...
def test_unnamed_container_is_found_by_recursive_search():
    """No `usage`/`tokens` key at all — the fallback has to find the token-shaped dict itself."""
...
def test_search_ignores_dicts_that_are_not_token_shaped():
    """A payload with an `input` that means something else (a tool's argument) must not be mistaken for usage."""
...
def test_opencode_steps_sum_into_one_turn():
    """The reason merge exists: opencode reports per step, and only the sum is the turn's consumption."""
...
def test_duration_is_replaced_not_summed():
    """Duration is a span of time, not a quantity — summing two reports of the same turn's elapsed time would double it."""
    total = usage.TurnUsage()
...
def test_empty_event_leaves_the_total_alone():
    """Most events in a stream are text/tool deltas."""
