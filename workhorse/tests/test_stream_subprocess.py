...
def test_silent_stream_still_emits_liveness_heartbeats():
    """The wedged case: a turn producing NO output must still report that it is alive and how long it has been quiet."""
...
def test_wedged_midline_is_killed_by_watchdog():
    """A process that writes a partial line (no newline) then sleeps forever must be force-killed within timeout+grace — before this fix, readline() blocked and the in-loop wall-clock check never ran again, hanging the node indefinitely."""
...
def test_the_turn_deadline_is_measured_on_the_injected_clock():
    """A two-hour timeout expires in about a second, because the supervisor is handed its clock rather than importing one."""
    ...
    timed_out, rc = supervisor.stream(
        ...
        7200.0,
        lambda _line: None,
        resilience=AgentResilience(watchdog_grace_s=3600),
    )
    elapsed = time.monotonic() - start
    assert timed_out is True
    assert rc != 0
...
def test_the_child_pwd_matches_the_cwd_it_was_spawned_in(tmp_path):
    """A CLI that resolves its project root from ``$PWD`` (OpenCode does) must land in the node's repo, not the one workhorse was launched from."""
...
def test_the_stale_oldpwd_is_dropped_when_the_child_moves(tmp_path):
    """``cd -`` in an agent's shell must not jump to the launcher's directory: the child never performed the ``cd`` that ``OLDPWD`` describes."""
    target = tmp_path / "target-repo"
    target.mkdir()
