...
def test_none_power_yields_the_profile_default_and_no_override():
    """No `power:` set on the node: the profile's `default` table provides model + effort."""
    with _config(CONFIG):
...
def test_a_backend_mismatch_returns_empty():
    """The resolver validates the narrowed profile's `cli` against the requested backend: an opencode-profile call against claude returns nothing rather than silently billing on the wrong CLI."""
    with _config(CONFIG):
...
def test_empty_config_keeps_harness_defaults_unset():
    """A bare-CLI run (empty cfg handed to the resolver) reads nothing — the CLI's own default model applies, and effort is whatever the CLI's invocation defaults to."""
    with _config({}):
...
def test_default_table_fills_powerless_node():
    """A node with no `power:` reads the profile's `default` table — the per-CLI fallback model/effort, which every backend implements."""
    with _config(CONFIG):
        assert _model_effort(None, "claude", None) == ("sonnet", "medium")
...
def test_a_profile_replaces_the_top_level_tables():
    """When `--profile local` is set, the resolvers see only the local profile — its `powers.high` answers for opencode, nothing else does."""
...
def test_a_switch_re_stamps_the_run_span_so_telemetry_says_what_it_spent_on():
    """The root span closes with the run, so a switch at hour two of a hundred is not a lost update — it is the correction that keeps a cross-run comparison honest."""
    runner = _runner()
...
def test_the_box_is_shared_so_a_sub_flow_cannot_put_the_parent_back():
    """The runner is frozen and a handoff copies the env around it, so a *string* here could not be reassigned at all — and a per-copy string would put the parent flow back on the old model set the moment the child returned."""
    runner = _runner()
    child = runner
    with _file(_SWITCHABLE):
