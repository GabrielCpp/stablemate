...
def test_a_fresh_run_does_not_inherit_the_previous_runs_node_output():
    """A benchmark run showed this exactly: a re-run that passed every story and never entered `flag_qa_failure` or `flag_epic_blocked` shipped a run dir containing both nodes' output, left by the failed run before it — while its own 57 events mention neither."""
...
def test_re_entering_a_flow_scope_does_not_inherit_the_previous_visits_answers():
    """`at()` is the same fresh start one level down, and there it bites *within* a single run: a flow node in a loop re-enters one scope per iteration — the coder graph hands off to `Qa` once per story, always onto `<run>/qa/_flow`."""
    with tempfile.TemporaryDirectory() as tmp:
...
def test_a_genuine_mid_flow_resume_still_re_enters_in_place():
    """`subscope(resume=True)` is the engine's "we died inside this exact node" signal, and it must survive the emptying above — otherwise a resume replays every agent turn the sub-flow had already paid for."""
    with tempfile.TemporaryDirectory() as tmp:
        parent = ArtifactWriter("coder", Path(tmp) / "runs", run_id="t")
...
def test_the_profile_and_what_it_held_are_recorded_on_the_run():
    """The name is what a resume re-applies; the tables are what answers "which model was at `high`" once the config file has moved on, which the name alone cannot."""
    with tempfile.TemporaryDirectory() as tmp:
...
def test_a_resume_carries_the_recorded_profile_rather_than_clearing_it():
    """`resume()` rewrites run.json before the driver has said anything about a profile, so a carried-over value is the difference between a record and a file that forgets what the run is running on every time it picks back up."""
    with tempfile.TemporaryDirectory() as tmp:
...
def test_the_run_dir_records_the_command_that_would_resume_it():
    """The record a watcher needs after the process it describes is gone."""
    with tempfile.TemporaryDirectory() as tmp:
...
def test_a_nested_flow_scope_records_no_launch_of_its_own():
    """`at()`/`subscope()` open a writer for a flow scope inside a run — `<run>/qa/_flow`, once per story."""
    with tempfile.TemporaryDirectory() as tmp:
        writer = ArtifactWriter("coder", Path(tmp) / "runs", run_id="pdeadbeef")
...
def test_an_unwritable_run_dir_does_not_take_the_run_down_with_it():
    """Best-effort, like every other record here."""
    with tempfile.TemporaryDirectory() as tmp:
        writer = ArtifactWriter("coder", Path(tmp) / "runs", run_id="pdeadbeef")
