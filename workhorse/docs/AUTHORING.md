...
turn's process tree. That is the engine's behaviour and it is not negotiable, so anything
that must survive across states (a dev server, a compose stack, an emulator) is brought up
by a `script` node from a manifest, health-gated, and reaped by a later one.

The primitive that does it is **`ostler.qa.stack`** — `ensure_stack` / `teardown_stack`,
taking a manifest dict the workflow supplies. It lives there rather than here because the
stacks that need it are the ones QA drives, and ostler is what drives QA; workhorse has no
consumer of its own. The manifest keys, the shell-recipe contract and the return shape are
in [ostler's QA-RUN.md](../../ostler/docs/QA-RUN.md#the-durable-stack-ostlerqastack).

What stays workhorse's problem is *when* you call it: from a `script` node, never from
inside an agent turn, or the teardown that reaps the turn takes the stack with it.
