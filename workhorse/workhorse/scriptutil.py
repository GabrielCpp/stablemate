...
def _repo_name_from_dir(path: Path) -> str:
    ...
    ``.../Globex`` and a config value ``globex`` resolve to the same key."""
...
def checkout_workspace(
    ...
    2. Otherwise (no workspace file set), synthesize a single folder from the existing
       ...
       bind-mounted path (Globex's read-only host-working-tree trick) works exactly
...
def get_repo_config(repo_name: str, key: str, default=None, *, repos: dict | None = None):
    ...
    Examples:
        get_repo_config("acme", "qa_mode")            # → "cli"
        get_repo_config("acme", "base_branch", "main") # → "develop"
    """
    if repos is None:
