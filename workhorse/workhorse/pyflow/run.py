    ...
    try:
        try:
            ...
            if isinstance(exc, RunBudgetExceeded | AgentTurnFailed):
                ...
                # `run.json`, no terminal, exit 1. An agent turn the ladder gave up on
                # is the same kind of fact — the provider quit, the workflow decided
                # nothing — and gets the same treatment, which is what the raw
                # `BackendInvocationError` below has always had.
                print(f"[workhorse] ERROR: {exc}")
