...
def run_agent(
    ...
    *,
    ...
    1. **Transient retries** (inside ``_run_turn_with_recovery``): rate limits,
       overloads, network blips, timeouts, *empty* results and spending caps are
       retried or waited out with backoff.
...
def _invoke_and_parse(
    ...
    *,
    ...
    for attempt in range(max_output_retries + 1):
        result_text = _run_turn_with_recovery(
        ...
        try:
            ...
            if is_cap_hit:
                if cap_waits >= resilience.max_cap_waits:
                    raise
                cap_waits += 1
                delay, when = cap_delay_seconds(exc, resilience=resilience)
                print(
                ...
                sleep_with_notice(delay, node_id, "cap reset", resilience=resilience)
